...
export default function ProgramPage() {
  return (
    <div className={`${spectral.variable} ${workSans.variable} font-[family-name:var(--font-work-sans)]`}>
      {/* Cover */}
      <section className="relative overflow-hidden bg-[#1c2b3a] px-6 py-24 sm:px-12 sm:py-32">
        <Image
          src="/assets/images/program/hero.webp"
          alt="Students learning together with a mentor"
          fill
          priority
          className="object-cover"
          sizes="100vw"
        />
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-[rgba(20,30,40,0.35)] via-[rgba(20,30,40,0.25)] to-[rgba(18,26,35,0.94)]" />
        <div className="relative mx-auto max-w-5xl">
          <div className="mb-8 flex items-center gap-3">
            <div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-[#faf6ef]">
              <Image src="/assets/images/program/logo.webp" alt="Mirror Intelligence" width={48} height={48} className="h-full w-full object-contain" />
            </div>
            <span className="text-sm font-semibold uppercase tracking-[0.16em] text-[#e8c9a0]">
          ...
          <div className="mb-8 flex flex-wrap gap-2.5">
            {['Available 24/7', 'Repeat exams until mastery', 'Registered UNEB candidate', 'Hear back in 3 hours'].map(
              (label) => (
                <span
                  key={label}
                  className="rounded-full border border-[#faf6ef]/40 bg-[#faf6ef]/[0.08] px-4 py-1.5 text-sm font-semibold text-[#faf6ef] backdrop-blur-sm"
                >
                  {label}
                </span>
              ),
          ...
          <h1 className="max-w-3xl text-balance font-[family-name:var(--font-spectral)] text-4xl font-semibold leading-[1.1] text-[#faf6ef] sm:text-6xl">
            The school that adapts to the learner, not the other way round.
          </h1>
          <div className="mt-8 flex flex-wrap items-center gap-4 text-base sm:text-lg">
            <span className="inline-flex items-center gap-2 font-bold text-[#faf6ef]">
              <span className="h-2 w-2 rounded-full bg-[#e8a15c]" />
              You are here — Uganda
            </span>
