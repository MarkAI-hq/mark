...
// src/app/(dashboard)/dashboard/settings/billing/_components/billing-client.tsx
...
// detectRegion and REGIONAL_PRICES are in billing-utils (not 'use server')
  ...
  return (
    <div className={`rounded-lg border p-3 space-y-2.5 ${
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Brain className={`h-4 w-4 shrink-0 ${
            isExhausted ? 'text-rose-500' : isWarning ? 'text-amber-500' : 'text-slate-500'
          }`} />
          <span className={`text-xs font-semibold ${
            isExhausted ? 'text-rose-700' : isWarning ? 'text-amber-700' : 'text-slate-700'
          }`}>
            AI Gradings This Month
          </span>
        </div>
        <span className={`text-xs font-bold tabular-nums ${
          isExhausted ? 'text-rose-600' : isWarning ? 'text-amber-600' : 'text-slate-600'
        }`}>
          {used} / {limit}
        </span>
      </div>
      ...
      <div className="flex items-center justify-between gap-2">
        <p className={`text-xs ${
          isExhausted ? 'text-rose-600' : isWarning ? 'text-amber-600' : 'text-slate-500'
        }`}>
          {isExhausted
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-slate-900">{PLAN_META[currentPlan].label}</p>
              <p className="text-xs text-slate-500">{PLAN_META[currentPlan].desc}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1 rounded-lg border bg-slate-50 p-1">
            {(['monthly', 'annual'] as const).map((i) => (
              ...
              >
                ...
                {i === 'annual' && (
                  <span className="ml-1.5 rounded-full bg-emerald-100 px-1.5 py-0.5 text-[10px] text-emerald-700 font-semibold">
                    -25%
                  </span>
                )}
              </button>
            ...
            >
              ...
              <CardContent className="flex flex-col flex-1 gap-4">
                ...
                {isCurrent ? (
                  ...
                  <Button variant="outline" size="sm" disabled className="w-full text-slate-400">
